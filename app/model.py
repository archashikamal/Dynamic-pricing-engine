...
def _predict(model, features: pd.Series) -> float:
    """Safe predict wrapper — fills NaN, casts dtype, handles array output."""
    arr = features.fillna(0).values.reshape(1, -1).astype(np.float64)
    out = model.predict(arr)
    # XGBoost ≥3.x returns ndarray; older versions may return scalar
...
def optimize_price(
    ...
    base_demand = _predict(model, features)
    ...
    opt_features = features.copy()
    opt_features["price"] = optimal_price
    opt_features["log_price"] = np.log1p(optimal_price)
    optimal_demand = _predict(model, opt_features)
